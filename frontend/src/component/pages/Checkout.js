...
const Checkout = ({ setOrderFormValue }) => {
    ...
    const [formValues, setFormValues] = useState({ firstname: "", lastname: "", email: "", phonenumber: "", streetaddress: "", country: "", postal: "", city: "", province: "", card: "", expiration: "", validationCode: "", cvv: "" });
    const [formErrors, setFormErrors] = useState({});
    const navigate = useNavigate();

    useEffect(() => {
        document.title = "Checkout | Modest Zone";
    }, []);
    ...
    const validateForm = (values, step = "payment") => {
        ...
        const validationCodeRegex = /^[0-9]{6}$/;
        const errors = {};
        ...
        if (step === "payment") {
            ...
            } else if (!validationCodeRegex.test(values.validationCode)) {
                errors.validationCode = "Validation code is invalid";
            }
            if (!values.cvv) {
    ...
    const submitForm = (e) => {
        ...
        if (Object.keys(validateForm(formValues)).length > 0) {
            ...
            setFormValues({ firstname: "", lastname: "", email: "", phonenumber: "", streetaddress: "", country: "", postal: "", city: "", province: "", card: "", expiration: "", validationCode: "", cvv: "" });
                            ...
                            currentStep === "payment" ?
                                <React.Fragment>
                                    ...
                                    <section className="checkout-inputs">
                                        ...
                                        />
                                        <span className="error">{formErrors.validationCode}</span>
                                        <input
                                            onChange={handleChange}
